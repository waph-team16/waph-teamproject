...
The miniFacebook app provides users with a registration process. Ensures a secure social networking environment. We have prioritized user data protection by implementing HTTPS deployment hashed passwords and role based access control. Additionally features like real time chat and superuser account management enhance user interaction and administrative capabilities.
...
![ Database system design](design.jpg)
...
![ Registration Page](loginpage.png)

# Database creation:

![ Registration Page](dbentity.png)
...
![Edit post page](editpostpage.png)
...
![Super User home page](adminhome.png)

# The registered users:

As we seperated the users and super users in database and also provided seperate login pages for them. A normal user cannot login from admin page.

Admin login page shown below is ony for admins

![Admin Login Page](adminloginpage.png) 
...
![Disabling user from Admin Homepage](disableuser.png) 

![Error when disabled user login](disablecheck.png) 


# Enable users:

A disabled user can be enabled again by super user therough homepage as shown below

![Disabling user from Admin Homepage](enableuser.png) 

enabled user can login again same as before

# Security analysis:

We have implemented certain prvention methodologies that includes **HTML ENTITIES** , ehich sanitizesthe user input. We also made application secure against XSS attacks, SQL Injections and CSRF attacks.
...
![Web application running over https](https.png) 

# Passwords being hashed:

passwords are hashed inside database

![Hashed passwords stored inside database](passwordhash.png) 

# SQL in Prepared Statements:

All sql scripts are stored in database-data.sql file as shown in below

![prepared statements in sql](preparedstatements.png) 

# Input validation:

Input from users is validated and sanitized in all layes as shown below

![input validations on registration page](htmlsanitization.png) 

![Sanitized db data](dbsanitization.png) 

![HTML ENTITIES used for input sanitization](htmlentities.png) 

# HTML being sanitized:

In all html code the code is sanitized and output i

![All html outputs are validated and sanitized](htmlvalidation.png) 

# Role based access:

Regular user cannot login as super user as their login pages and tdatabase tables are seperated.

![Regular user attempt to login as superuser](wrongpwd.png) 

Normal user cannot edit or delete posts of other users when the attept system throws below error 

![Post edit failed by other user](editpostfail.png) 

![Post delete attempt by other user](deletefail.png) 

# Prevention of Session Authentication and Hijacking:

![Session Hijacking test on Web application](sessionhijackscreenshot.png) 

# CSRF Protection of website:

CSRF check is performed against the web app and resulted image is shown below

![CSRF attack check](csrfprotect.png) 

# Open source template of CSS:
 
![CSRF attack check](cssTemplet.png)  

# A team project website:

![WAPH-team16 Website](projectwebsite.png) 
...
Sprint 0: 11:59 pm March 24
Links to an external site.
Sprint 0-Getting Started assignment aims to foster initial collaboration among team members using Scrum practices, enabling your team to become acquainted and prepare collaboratively for the entire project's scope.
Sprint 1: 11:59 pm March 31

Sprint 2: 11:59 pm April 16 (changed from 11)

Sprint 3: No submission
Links to an external site.
A super user can disable/enable an account
